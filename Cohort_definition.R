...
# When there are more the 367 samples available, they will be randomly selected from available samples. 

# Libraries
library(SummarizedExperiment)

### GBM ###
gbm_cohort <- read.csv("2.0/Expressão Pan/Expression/total/expression_gbm.csv", row.names = 1) #Previous expression table (genes x samples)
md_gbm_cohort <- read.csv("2.0/Expressão Pan/Metadata/total/metadata_gbmR_f.csv") #Metadata table after manual curation (in .csv) (sample x metadata)
...
gbm_normal_cohort <- read.csv("2.0/Expressão Pan/Expression/total/matrix_gbmR_normal.csv", row.names = 1)
...
md_gbm_cohort <- md_gbm_cohort[, c("barcode", "tissue_or_organ_of_origin", "condition", "project")] #Should rest only 4 variables
...
gbm_cohort  <- gbm_cohort [rownames(gbm_cohort ) %in% ensembl_genes, ] #dds filtered by ensembl ID
matrix_comb  <- matrix_comb[rownames(matrix_comb) %in% ensembl_genes, ] #dds filtered by ensembl ID

  # Binding tables
matrix_comb <- matrix_comb[rownames(gbm_cohort), ] #Reorder the rawnames to match
...
### BRCA ###
brca_cohort <- read.csv("2.0/Expressão Pan/Expression/total/expression_brca.csv", row.names = 1) #Previous expression table (genes x samples)
md_brca_cohort <- read.csv("2.0/Expressão Pan/Metadata/total/metadata_brcaR_f.csv") #Metadata table after manual curation (in .csv) (sample x metadata)
...
brca_normal_cohort <- read.csv("2.0/Expressão Pan/Expression/total/matrix_brcaR_normal.csv", row.names = 1)
md_brca_normal_cohort <- read.csv("2.0/Expressão Pan/Metadata/total/metadata_brcaR_normal.csv")
...
md_brca_cohort <- md_brca_cohort[, c("barcode", "tissue_or_organ_of_origin", "condition", "project")] #Should rest only 4 variables
...
luad_normal_cohort <- read.csv("2.0/Expressão Pan/Expression/total/matrix_luadR_normal.csv", row.names = 1)
md_luad_normal_cohort <- read.csv("2.0/Expressão Pan/Metadata/total/metadata_luadR_normal.csv")
...
md_luad_cohort <- md_luad_cohort[, c("barcode", "tissue_or_organ_of_origin", "condition", "project")] #Should rest only 4 variables
...
metadata_luadR <- rbind(md_lung_sel, md_luad_cohort_sel)
  ...
  #Saving tables
write.csv(matrix_luadR, "2.0/Expressão Pan/Expression/cohort/matrix_luadR_comb.csv", row.names = TRUE) #expression matrix
write.csv(metadata_luadR, "2.0/Expressão Pan/Metadata/cohort/metadata_luadR_comb.csv", row.names = TRUE) #metadata

### COAD ###
coad_cohort <- read.csv("2.0/Expressão Pan/Expression/total/expression_coad.csv", row.names = 1) #Previous expression table (genes x samples)
md_coad_cohort <- read.csv("2.0/Expressão Pan/Metadata/total/metadata_coadR_f.csv") #Metadata table after manual curation (in .csv) (sample x metadata)
...
coad_normal_cohort <- read.csv("2.0/Expressão Pan/Expression/total/matrix_coadR_normal.csv", row.names = 1)
md_coad_normal_cohort <- read.csv("2.0/Expressão Pan/Metadata/total/metadata_coadR_normal.csv")
...
md_coad_cohort <- md_coad_cohort[, c("barcode", "tissue_or_organ_of_origin", "condition", "project")] #Should rest only 4 variables
